...
def grouped_bootstrap_interval(
    ...
    """Bootstrap complete image groups because paired rows from one image are dependent."""
...
def main() -> None:
    ...
    if "generated" in set(validation["source"]):
        raise SystemExit("Validation must contain real images only")
    ...
    real_mask = train["source"].ne("generated").to_numpy()
    multimodal_jobs = [
        (
            "PyTorch multimodal CNN (real-only training)",
            REAL_ONLY_SLUG,
            f"{int(train.loc[real_mask, 'image_id'].nunique())} real train images",
            real_mask,
            RANDOM_STATE + 2,
        ),
        (
            ...
            SYNTHETIC_SLUG,
            (
                f"{int(train.loc[real_mask, 'image_id'].nunique())} real + "
                f"{int(train.loc[~real_mask, 'image_id'].nunique())} synthetic train images"
            ),
    ...
    run_info = {
        "result_source": "Generated by python -m src.train from Dataset V2 project files.",
        "dataset_version": "2.0",
        ...
        "model_selection_split": (
            f"data/validation.csv ({int(validation['image_id'].nunique())} independent real images)"
        ),
        ...
        "training_real_images": int(train.loc[train["source"].ne("generated"), "image_id"].nunique()),
        "training_synthetic_images": int(train.loc[train["source"].eq("generated"), "image_id"].nunique()),
